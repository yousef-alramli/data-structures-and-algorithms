...
function transformToLis(obj) {
  ...
  let htmlElements = values.map((item, index) => {
...
const totalSum = (input) => {
  ...
  input.forEach(element => {
    summation = element.reduce((pre, cur) => pre + cur, summation)
  });
  return summation;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let result = input.map(int => {
    return int.filter(num => num % 5 === 0)
  })
  let power = result.map(num => Math.pow(2,num));
  return  power
};
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(item => {
    if (item.gender == 'male' || item.gender == 'female') {
      genedered.push(item.name);
    }
  });

  return genedered.join(' and ');
};
