...
const screenForNames = (arr) => {
 ...
 let array =arr.filter(item => regex.test(item));
  return array;
}
...
const toTitleCase = (arr) => {
  // Solution code here...
  let array = arr.map(item=> item.charAt(0).toUpperCase() + item.slice(1))
  return array
};
...
let biggerThanLuke = (arr) => {
  ...
  let array = arr.filter(element => element.mass > 77);
  let nameArr = array.map(item => item.name);
  return nameArr.join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let sorting = arr.sort((value, value2) => {
