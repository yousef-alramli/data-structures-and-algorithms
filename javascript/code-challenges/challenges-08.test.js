...
const sortByChildren = (charArray) => {
  // Solution code here...
  let arrange = charArray.sort((prev, curr) => {
...
const isCapitalized = (str) => {
  ...
  let expresstion = str.match(/\b[A-Z](\w)*/g);
  if (expresstion == null) {
    return [];
  }
  return expresstion;
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let citiName = /\b[A-J]/g;
  let array = arr.filter((city) => {
    if (citiName.test(city[0])) {
      return city;
    } else {
      return false;
    }
