...
const longestString = (arr) => {
  ...
  let sorting = [...arr].sort((a, b) => b.length - a.length);
  let longest = arr.indexOf(sorting[0])
  return longest;
};
...
const findHappiness = (arr) => {
  ...
  let array = [];
  arr.forEach(item => {
    let str = item.includes(":)") ? array.push(item) : '';
...
const standardizePhoneNumbers = (arr) => {
  ...
  let array = [];
  arr.forEach(item => {
    array.push(item.substring(1, 4) + item.substring(6, 9) + item.substring(10, 14))
  });
  return array;
};
...
const onlyOddChars = (str) => {
 ...
 let ArrayStr = str.split('')
  ...
  ArrayStr.map((item,index) => {
    let filtered = index%2 ==0 ? '' : newStr.push(item);
...
const allHappy = (arr) => {
  ...
  let array = [];
  arr.forEach(item => {
    item.includes(":)") ? array.push(true) :array.push(false);

  })
let boolean = array.includes(false) ? false : true;
  return boolean;
};
