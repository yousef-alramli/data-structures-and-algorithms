...
const forEachTwoToThe = (arr) => {
  ...
  let pwdArr =[];
  arr.forEach(pwdNums => {
    pwdArr.push(Math.pow(2,pwdNums))
  });
  return pwdArr;
};
...
const mapTwoToThe = (arr) => {
  ...
  const array = arr.map(num=>Math.pow(2, num))
  return array;
};
...
const charCode = (arr) => {
  // Solution code here...
  const array = arr.map(nums=>nums.String.charCodeAt(0));
  return array;
};
